#!/usr/bin/env bash
set -euo pipefail
OUT=/tmp/proj-esp32-update-scheduler-build.txt
: > "$OUT"
...
rm -f scripts/integrate_update_scheduler.py scripts/integrate_update_scheduler_job.sh scripts/run_scheduler_integration_job.sh
