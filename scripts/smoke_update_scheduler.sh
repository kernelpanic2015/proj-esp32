...
python3 scripts/release_manifest.py --firmware .pio/build/nodemcu-32s-remote-update-test/firmware.bin --output "$TRANS" --model proj-esp32-35 --hardware-revision 1 --version 0.1.26-remote-test --build 27 --channel dev --private-key "$KEYDIR/update-signing-private.pem" --public-key "$KEYDIR/update-signing-public.pem" >/dev/null
python3 scripts/release_manifest.py --firmware .pio/build/nodemcu-32s-remote-target-test/firmware.bin --output "$TARGET" --model proj-esp32-35 --hardware-revision 1 --version 0.1.27 --build 28 --channel dev --private-key "$KEYDIR/update-signing-private.pem" --public-key "$KEYDIR/update-signing-public.pem" >/dev/null
...
wait_for_transition '0.1.26-remote-test' 27 app1
...
sleep 3
wait_for_online '0.1.26-remote-test' 27
P=$(curl -fsS --max-time 5 "http://$ESP_HOST/api/update/policy")
SCH=$(curl -fsS --max-time 5 "http://$ESP_HOST/api/update/scheduler")
...
for _ in $(seq 1 90); do
  ...
  if grep -q '"state":"AVAILABLE"' <<<"$R" && grep -q '"package_prepared":true' <<<"$U" && grep -q '"candidate_build":28' <<<"$U" && grep -Eq '"attempt_count":[1-9][0-9]*' <<<"$SCH" && grep -Eq '"accepted_count":[1-9][0-9]*' <<<"$SCH"; then
    AVAILABLE=1
...
wait_for_transition '0.1.27' 28 app0
