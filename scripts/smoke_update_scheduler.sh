...
python3 scripts/release_manifest.py --firmware .pio/build/nodemcu-32s-remote-update-test/firmware.bin --output "$TRANS" --model proj-esp32-35 --hardware-revision 1 --version 0.1.17-remote-test --build 18 --channel dev --private-key "$KEYDIR/update-signing-private.pem" --public-key "$KEYDIR/update-signing-public.pem" >/dev/null
python3 scripts/release_manifest.py --firmware .pio/build/nodemcu-32s-remote-target-test/firmware.bin --output "$TARGET" --model proj-esp32-35 --hardware-revision 1 --version 0.1.18 --build 19 --channel dev --private-key "$KEYDIR/update-signing-private.pem" --public-key "$KEYDIR/update-signing-public.pem" >/dev/null
...
wait_for_transition '0.1.17-remote-test' 18 app0
...
sleep 3
wait_for_online '0.1.17-remote-test' 18
P=$(curl -fsS --max-time 5 "http://$ESP_HOST/api/update/policy")
SCH=$(curl -fsS --max-time 5 "http://$ESP_HOST/api/update/scheduler")
...
for _ in $(seq 1 90); do
  ...
  if grep -q '"state":"AVAILABLE"' <<<"$R" && grep -q '"package_prepared":true' <<<"$U" && grep -q '"candidate_build":19' <<<"$U" && grep -Eq '"attempt_count":[1-9][0-9]*' <<<"$SCH" && grep -Eq '"accepted_count":[1-9][0-9]*' <<<"$SCH"; then
    AVAILABLE=1
...
wait_for_transition '0.1.18' 19 app1
