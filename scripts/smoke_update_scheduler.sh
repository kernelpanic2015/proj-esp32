...
python3 scripts/release_manifest.py --firmware .pio/build/nodemcu-32s-remote-update-test/firmware.bin --output "$TRANS" --model proj-esp32-35 --hardware-revision 1 --version 0.1.23-remote-test --build 24 --channel dev --private-key "$KEYDIR/update-signing-private.pem" --public-key "$KEYDIR/update-signing-public.pem" >/dev/null
...
sleep 3
wait_for_online '0.1.23-remote-test' 24
P=$(curl -fsS --max-time 5 "http://$ESP_HOST/api/update/policy")
SCH=$(curl -fsS --max-time 5 "http://$ESP_HOST/api/update/scheduler")
...
for _ in $(seq 1 90); do
  ...
  if grep -q '"state":"AVAILABLE"' <<<"$R" && grep -q '"package_prepared":true' <<<"$U" && grep -q '"candidate_build":25' <<<"$U" && grep -Eq '"attempt_count":[1-9][0-9]*' <<<"$SCH" && grep -Eq '"accepted_count":[1-9][0-9]*' <<<"$SCH"; then
    AVAILABLE=1
